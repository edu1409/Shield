...
//        public void Welcome()
//        {
//            var message = Constants.LCD20x4_WELCOME;
...
//        {
//            _cursor = new() { Left = 0, Top = 0 };
//            _displayService.Write(Constants.LCD20x4_TITLE, _cursor);
//            _cursor.Top = 1;
//            _displayService.Write($"{DateTimeOffset.Now:dd/MM/yyyy HH:mm}", _cursor);

//            _logger.LogInformation(Constants.LCD_DATE_TIME_UPDATED);
//        }
...
//            _displayService.Write($"Temp: {(sensorReading.Temperature.Kelvins.Equals(0) ? Constants.LCD_ERROR_STATE
//                : sensorReading.Temperature.DegreesCelsius.ToString("0.#") + (char)1 + "C")}", _cursor);

//            _cursor.Top = 3;
//            _displayService.Write($"Umid: {(sensorReading.RelativeHumidity.Equals(RelativeHumidity.Zero, RelativeHumidity.Zero) ? Constants.LCD_ERROR_STATE
//                : sensorReading.RelativeHumidity.Percent.ToString("#.##") + "%")}", _cursor);

//            _logger.LogInformation(Constants.LCD_CLIMATIC_INFO_UPDATED);
//        }
